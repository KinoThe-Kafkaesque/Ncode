/**
 * Autoresearch pure helpers (ported verbatim from oh-my-pi
 * `autoresearch/helpers.ts`).
 *
 * Metric/ASI parsing of harness stdout, MAD-noise-floor confidence inputs
 * (`isBetter`), number formatting, path-spec matching, and ASI sanitization.
 * The git-status convenience wrappers (`tryGitStatus`/`tryGitPrefix`) live in
 * `git.ts` here since ncode shells out to git directly.
 */

import type { ASIData, ASIValue, MetricDirection, NumericMetricMap } from './types.js'
