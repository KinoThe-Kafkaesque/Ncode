...
function optionalStringFrom(value: unknown): string | null {
...
function updateConfig(state: ReconstructedJsonlState, entry: AutoresearchConfigEntry): void {
  ...
  if (entry.goal !== undefined) state.goal = optionalStringFrom(entry.goal);
  if (entry.scopePaths !== undefined) state.scopePaths = stringArrayFrom(entry.scopePaths);
  if (entry.offLimits !== undefined) state.offLimits = stringArrayFrom(entry.offLimits);
  if (entry.constraints !== undefined) state.constraints = stringArrayFrom(entry.constraints);
  if (entry.maxIterations !== undefined) {
    state.maxIterations = typeof entry.maxIterations === 'number' ? entry.maxIterations : null;
  }
  if (entry.baselineCommit !== undefined) state.baselineCommit = optionalStringFrom(entry.baselineCommit);
}
...
function runFrom(entry: AutoresearchRunEntry, segment: number): ReconstructedRun {
  return {
    ...
    modifiedPaths: stringArrayFrom(entry.modifiedPaths),
    scopeDeviations: stringArrayFrom(entry.scopeDeviations),
    justification: optionalStringFrom(entry.justification),
    flagged: entry.flagged === true,
    flaggedReason: optionalStringFrom(entry.flaggedReason),
  };
}
