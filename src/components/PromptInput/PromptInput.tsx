...
import { usePermissionModeMachine } from './permissionModeMachine.js';
...
import { isUltrathinkEnabled } from '../../utils/thinking.js';
...
import {
  createPromptInputFooterBindingHandlers,
} from './promptInputFooterBindingDispatch.js';
import {
...
import { usePromptInputDisplayModel } from './promptInputDisplayModel.js';
import { usePromptInputFooterController } from './usePromptInputFooterController.js';
import { dispatchPromptInputAgentRoute, dispatchPromptInputDirectMessageShortcut } from './promptInputSubmitRouteDispatch.js';
import { insertTextAtCursorState } from './promptInputDraftMutationPlan.js';
import {
...
function PromptInput({
  ...
  recordRenderTrace('PromptInput');
  // Synchronous mode ref prevents race between onChange mode switch
  // and onSubmit reading a stale closure-captured mode variable. React
  // may not have committed the setMode state update before Enter fires.
  ...
  // Tmux pill — visible when Tungsten has an active persistent session.
  const tungstenEnabled = process.env.NCODE_BUILD_MODE === 'noumena' || process.env.USER_TYPE === 'noumena';
  const hasTungstenSession = useAppState(s => tungstenEnabled && s.tungstenActiveSession !== undefined);
  const tmuxFooterVisible = tungstenEnabled && hasTungstenSession;
  ...
  if (nextPasteIdRef.current === -1) {
    nextPasteIdRef.current = getInitialPasteId(messagesRef.current);
  }
  ...
  const hasBgTaskPill = useMemo(() => Object.values(tasks).some(t => isBackgroundTask(t) && !(isInternalBuild() && isPanelAgentTask(t))), [tasks]);
  ...
  const transcriptClassifierEnabled = feature('TRANSCRIPT_CLASSIFIER') ? true : false;
  const {
    showAutoModeOptIn,
    handleCycleMode,
    handleAutoModeOptInAccept,
    handleAutoModeOptInDecline,
  } = usePermissionModeMachine({
    toolPermissionContext,
    teamContext,
    viewingAgentTaskId,
    viewedTeammate,
  ...
  } = usePromptInputFooterController({
    tasks,
  ...
  const ultraplanEnabled = feature('ULTRAPLAN')
  ...
  const {
    ...
    teamContext,
    voiceInterimRange,
    slackChannelsEnabled: hasSlackMcpServer(store.getState().mcp.clients),
    ultraplanEnabled
  });
  ...
  useEffect(() => {
    if (feature('ULTRAPLAN') && ultraplanTriggers.length) {
      addNotification({
        ...
        text: 'This prompt will launch an ultraplan session in NCode on the web',
        priority: 'immediate',
        timeoutMs: 5000
      });
    } else {
  ...
  useEffect(() => {
    if (isUltrareviewEnabled() && ultrareviewTriggers.length) {
      addNotification({
        ...
        text: 'Run /ultrareview after NCode finishes to review these changes in the cloud',
        priority: 'immediate',
        timeoutMs: 5000
      });
    }
  ...
  const onChange = useCallback((value: string) => {
    ...
    // Race fix: onModeChange schedules a React state update, but
    // Enter may fire before the render propagates. Override the ref
    // synchronously so onSubmit sees the latest mode.
    if (plan.kind === 'change_mode_and_input') {
      modeRef.current = plan.nextMode;
    }
  }, [trackAndSetInput, onModeChange, input, cursorOffset, pushToBuffer, pastedContents, dismissStashHint, clearFooterSelection]);
  const {
    resetHistory,
    onHistoryUp,
    onHistoryDown,
  ...
  function handleHistoryUp() {
    dispatchPromptInputHistoryUp({
      ...
      isCursorOnFirstLine,
      hasEditableQueuedCommand: queuedCommands.some(isQueuedCommandEditable),
    }, {
  ...
  function handleHistoryDown() {
    dispatchPromptInputHistoryDown({
      ...
      isCursorOnLastLine,
      footerItems,
      hasSeenTasksHint: getGlobalConfig().hasSeenTasksHint,
    }, {
      onHistoryDown,
      selectFooterItem,
      markTasksHintSeen: () => {
  ...
  const onSubmit = useCallback(async (inputParam: string, isSubmittingSlashCommand = false) => {
    ...
    await dispatchPromptInputSubmit({
      ...
      footerSelectionVisible: isFooterSelectionVisible(state.footerSelection),
      viewSelectionMode: state.viewSelectionMode,
      hasImages,
      suggestions: suggestionsState.suggestions,
      isSubmittingSlashCommand,
      promptSuggestionState: {
        text: promptSuggestionState.text,
      ...
      agentRouteDeps: {
        ...
        onRouted: () => {
          logEvent('ncode_transcript_input_to_teammate', {});
        },
        onAgentSubmit
      },
      removeNotification,
  ...
  const promptSuggestionRenderState = useMemo(() => derivePromptSuggestionRenderState({
    mode,
    suggestionsCount: suggestions.length,
    promptSuggestion,
    promptSuggestionState,
    viewingAgentTaskId
  }), [mode, suggestions.length, promptSuggestion, promptSuggestionState, viewingAgentTaskId]);
  const showPromptSuggestion = promptSuggestionRenderState.showPromptSuggestion;
  useEffect(() => {
    if (!promptSuggestionRenderState.shouldMarkShown) {
      return;
    }
    markShown();
  }, [markShown, promptSuggestionRenderState.shouldMarkShown]);
  useEffect(() => {
    if (!promptSuggestionRenderState.shouldSuppressTiming || !promptSuggestionState.text) {
      return;
    }
  ...
  }, [promptSuggestionRenderState.shouldSuppressTiming, promptSuggestionState.text, setAppState]);
  function onImagePaste(image: string, mediaType?: string, filename?: string, dimensions?: ImageDimensions, sourcePath?: string) {
    const result = dispatchPromptInputImagePaste({
      ...
      pendingSpaceAfterPill: pendingSpaceAfterPillRef.current,
    }, {
      logImagePaste: () => {
        logEvent('ncode_paste_image', {});
      },
      setModePrompt: () => {
        onModeChange('prompt');
      },
    ...
    nextPasteIdRef.current = result.nextPasteId;
    pendingSpaceAfterPillRef.current = result.pendingSpaceAfterPill;
  }
  ...
  function insertTextAtCursor(text: string) {
    ...
    trackAndSetInput(insertPlan.nextInput);
  ...
  const popAllCommandsFromQueue = useCallback((): boolean => {
    return dispatchPromptInputQueuedCommandRestore({
      result: popAllEditable(input, cursorOffset),
      existingPastedContents: pastedContents,
    }, {
      setInputValue: trackAndSetInput,
      setModePrompt: () => {
        onModeChange('prompt');
      },
      setCursorOffset,
  ...
  }, [trackAndSetInput, onModeChange, input, cursorOffset, pastedContents, setPastedContents]);
  ...
  const onIdeAtMentioned = function (atMentioned: IDEAtMentioned) {
    logEvent('ncode_ext_at_mentioned', {});
  ...
  const handleUndo = useCallback(() => {
    if (canUndo) {
      const undoPlan = resolvePromptInputUndoPlan(undo());
      if (undoPlan.kind === 'restore') {
        trackAndSetInput(undoPlan.nextInput);
        setCursorOffset(undoPlan.nextCursorOffset);
        setPastedContents(undoPlan.nextPastedContents);
      }
    }
  ...
  const handleExternalEditor = useCallback(async () => {
    await dispatchPromptInputExternalEditor({
      ...
      logEditorUsed: () => {
        logEvent('ncode_external_editor_used', {});
      },
      setExternalEditorActive: setIsExternalEditorActive,
      editPromptInEditorImpl: editPromptInEditor,
      addNotification,
  ...
  const footerKeyHandlers = useMemo(() => createPromptInputFooterBindingHandlers({
    tasksSelected,
    isAnthropicUser: isInternalBuild(),
    coordinatorTaskCount,
    coordinatorTaskIndex,
    minCoordinatorIndex,
    isTeammateMode,
    teammateCount: inProcessTeammates.length,
    viewSelectionMode,
    footerItemSelected,
    ...
    cycleTeammateForward: totalAgents => {
      setTeammateFooterIndex(prev => (prev + 1) % totalAgents);
    },
    cycleTeammateBackward: totalAgents => {
    ...
    getTeammateIdAtIndex: index => inProcessTeammates[index]?.id,
    getVisibleTaskIdAtIndex: index => getVisibleAgentTasks(tasks)[index]?.id,
    getVisibleTaskAtIndex: index => getVisibleAgentTasks(tasks)[index],
    openBashesDialog: () => {
  ...
  }), [tasksSelected, coordinatorTaskCount, coordinatorTaskIndex, minCoordinatorIndex, isTeammateMode, inProcessTeammates, viewSelectionMode, footerItemSelected, teammateFooterIndex, viewingAgentTaskId, input, cursorOffset, navigateFooter, onSubmit, setAppState, tasks, onChange, setShowBashesDialog, setShowTeamsDialog, setShowBridgeDialog]);
  useKeybindings(footerKeyHandlers, {
    context: 'Footer',
  ...
  useInput((char, key) => {
    ...
    if (dispatchPromptInputUseInput({
      ...
      cursorOffset,
      helpOpen,
      speculationActive: speculation.status === 'active',
      sideQuestionVisible: !!(isSideQuestionVisible && onDismissSideQuestion),
      hasEditableQueuedCommand: queuedCommands.some(isQueuedCommandEditable),
      hasMessages,
      isLoading,
    }, {
      applyInputChange: onChange,
      setCursorOffset,
      setModePrompt: () => {
        onModeChange('prompt');
      },
  ...
  const handleModelSelect = useCallback((model: string | null, _effort: EffortLevel | undefined) => {
    ...
    logEvent('ncode_model_picker_hotkey', {
  ...
  const handleThinkingSelect = useCallback((enabled: boolean) => {
    ...
    logEvent('ncode_thinking_toggled_hotkey', {
      enabled
    });
    addNotification({
  ...
  if (showBashesDialog) {
    return <BackgroundTasksDialog onDone={() => setShowBashesDialog(false)} toolUseContext={getToolUseContext(messagesRef.current, [], new AbortController(), mainLoopModel)} initialDetailTaskId={typeof showBashesDialog === 'string' ? showBashesDialog : undefined} />;
  }
  ...
  return <Box flexDirection="column" marginTop={briefOwnsGap ? 0 : 1}>
    ...
    <Box position="absolute" marginTop={briefOwnsGap ? -2 : -1} height={suggestions.length === 0 && !showAutoModeOptIn ? 1 : 0} width="100%" paddingLeft={2} paddingRight={1} flexDirection="column" justifyContent="flex-end" overflow="hidden">
          <Notifications apiKeyStatus={apiKeyStatus} autoUpdaterResult={autoUpdaterResult} debug={debug} isAutoUpdating={isAutoUpdating} verbose={verbose} messagesRef={messagesRef} lastApiUsageKey={lastApiUsageKey} onAutoUpdaterResult={onAutoUpdaterResult} onChangeIsUpdating={setIsAutoUpdating} ideSelection={ideSelection} mcpClients={mcpClients} isInputWrapped={isInputWrapped} />
        </Box> : null}
    </Box>;
}
