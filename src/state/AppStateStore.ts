...
export type AppState = DeepImmutable<{
  ...
  // Remote session WS state (`ncode assistant` viewer). 'connected' means the
  ...
  remoteConnectionStatus:
    ...
    | 'reconnecting'
    | 'disconnected'
  // `ncode assistant`: count of background tasks (Agent calls, teammates,
  ...
  // Always-on bridge: ingress WebSocket is open (= "Connected" - user active on remote)
  ...
  // Always-on bridge: session URL on the remote web UI (set when connected)
  ...
  // True when the current turn was triggered by the "orchestrate" keyword.
  // Set on submit, cleared when isLoading goes false and no local_agent tasks remain.
  ...
  // Persistent JavaScript REPL kernel contexts keyed by tool name. REPL uses a
  // transparent orchestration kernel; js_repl uses a separate explicit kernel.
  javascriptReplContexts?: Map<
    string,
    {
  ...
  >
  pythonReplContexts?: Map<
    string,
    import('../tools/REPLTool/pyReplFactory.js').PythonReplManager
  >
  teamContext?: {
    teamName: string
    teamFilePath: string
