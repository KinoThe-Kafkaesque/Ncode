...
export async function handlePromptSubmit(
  ...
  let finalInput = expandPastedTextRefs(input, pastedContents)
  if (mode === 'bash' && finalInput.startsWith('!')) {
  ...
  logEvent('ncode_paste_text', { pastedTextCount, pastedTextBytes })
  ...
  if (!skipSlashCommands && finalInput.trim().startsWith('/')) {
    ...
    if (
      ...
      const onDone: LocalJSXCommandOnDone = (result, options) => {
        ...
        logForDebugging('[ncode-debug] executeUserInput no newMessages after processUserInput')
  ...
  try {
    ...
    await runWithWorkload(turnWorkload, async () => {
      ...
      if (newMessages.length) {
        ...
        logForDebugging(`[ncode-debug] executeUserInput before onQuery shouldQuery=${shouldQuery} messages=${newMessages.length} allowedTools=${(allowedTools ?? []).length}`)
        await onQuery(
          newMessages,
          abortController,
          shouldQuery,
