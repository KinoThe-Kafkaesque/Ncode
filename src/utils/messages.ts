...
export function withMemoryCorrectionHint(message: string): string {
  if (
    isAutoMemoryEnabled() &&
    getFeatureValue_CACHED_MAY_BE_STALE('ncode_amber_prism', false)
  ) {
...
export function DONT_ASK_REJECT_MESSAGE(toolName: string): string {
  return `Permission to use ${toolName} has been denied because Code is running in don't ask mode. ${DENIAL_WORKAROUND_GUIDANCE}`
}
 ...
 * This lets NCode reference message IDs when calling the snip tool.
 */
    ...
    .forEach(message => {
      switch (message.type) {
        ...
        case 'user': {
          ...
          //
          // Gated OFF when ncode_toolref_defer_j8m is active — that gate
...
export function mergeUserMessages(a: UserMessage, b: UserMessage): UserMessage {
  ...
  // Metadata merge: later message wins for tool-result fields.
  const mergedMeta = {
    ...a,
    sourceToolAssistantUUID:
      b.sourceToolAssistantUUID ?? a.sourceToolAssistantUUID,
    toolUseResult: b.toolUseResult ?? a.toolUseResult,
    permissionMode: b.permissionMode ?? a.permissionMode,
    origin: b.origin ?? a.origin,
    mcpMeta: b.mcpMeta ?? a.mcpMeta,
    imagePasteIds: [
      ...new Set([
        ...(a.imagePasteIds ?? []),
        ...(b.imagePasteIds ?? []),
...
export function mergeUserContentBlocks(
  ...
  if (!checkStatsigFeatureGate_CACHED_MAY_BE_STALE('ncode_chair_sermon')) {
    ...
    switch (contentBlock.type) {
      case 'tool_use': {
        ...
        if (typeof contentBlock.input === 'string') {
          ...
          if (parsed === null && contentBlock.input.length > 0) {
            ...
            logEvent('ncode_tool_input_json_parse_fail', {
      ...
      case 'text':
        if (contentBlock.text.trim().length === 0) {
          logEvent('ncode_model_whitespace_response', {
...
const STRIPPED_TAG_NAMES =
  'commit_analysis|context|function_analysis|pr_analysis|system-reminder|turn_aborted'
const STRIPPED_TAGS_RE = new RegExp(
  `<(${STRIPPED_TAG_NAMES})>.*?</\\1>\\n?`,
  'gs',
)
const DANGLING_OPEN_STRIPPED_TAG_RE = new RegExp(
  `<(${STRIPPED_TAG_NAMES})>[\\s\\S]*$`,
  'g',
)
const DANGLING_CLOSE_STRIPPED_TAG_RE = new RegExp(
  `</(${STRIPPED_TAG_NAMES})>\\n?`,
  'g',
)
...
export function stripPromptXMLTags(content: string): string {
  return content
    .replace(STRIPPED_TAGS_RE, '')
    .replace(DANGLING_OPEN_STRIPPED_TAG_RE, '')
    .replace(DANGLING_CLOSE_STRIPPED_TAG_RE, '')
    .trim()
}
...
export function filterUnresolvedToolUses(messages: Message[]): Message[] {
  ...
  if (unresolvedIds.size === 0 && orphanedToolResultIds.size === 0) {
    return messages
  }
  ...
  for (const msg of messages) {
    ...
    if (msg.type === 'assistant') {
      ...
      // Keep assistant only if NOT ALL tool_uses are unresolved
      if (
        toolUseBlockIds.length === 0 ||
        !toolUseBlockIds.every(id => unresolvedIds.has(id))
      ) {
        result.push(msg)
      }
      continue
    ...
    if (msg.type === 'user') {
      const filteredContent = content.filter(
        b => !(b.type === 'tool_result' && orphanedToolResultIds.has(b.tool_use_id)),
      )
      if (filteredContent.length === 0) {
        // Entirely orphaned tool_results — drop message
        continue
      }
...
export function handleMessageFromStream(
  ...
  if (
    ...
    if (message.type === 'assistant') {
      ...
      // When a committed assistant message contains a tool_use block, remove the
      // matching streaming preview entry immediately. This prevents the synthetic
      // streaming message and the real committed message from rendering together
      // during the window between content_block_stop and message_stop.
      ...
      if (toolUseIds.length > 0) {
        onStreamingToolUses(prev =>
          prev.filter(
            streamingToolUse =>
              !toolUseIds.includes(streamingToolUse.contentBlock.id),
          ),
        )
      }
    }
    // onMessage completes the stream turn.  The consumer (REPL) atomically
    // clears streamingText inside the same setMessages update that appends
    // the final message, so the streaming preview and final message never
    // render together regardless of reconciler batching behavior.
    onMessage(message)
    return
  }
  ...
  switch (message.event.type) {
    ...
    case 'content_block_stop': {
      // Remove the completed content block from streaming previews so the
      // synthetic message disappears as soon as the block is finalized,
      // rather than waiting for message_stop. This narrows the
      // inconsistent-render window to zero for the common path.
      const index = message.event.index
      onStreamingToolUses(prev => prev.filter(t => t.index !== index))
      return
    }
    case 'message_delta':
      onSetStreamMode('responding')
...
You have exited plan mode. You can now make edits, run tools, and take actions.${planReference}${attachment.verificationInstruction ?? ''}`
...
function filterTrailingThinkingFromLastAssistant(
  ...
  logEvent('ncode_filtered_trailing_thinking_block', {
    messageUUID:
...
export function filterWhitespaceOnlyAssistantMessages(
  ...
  const filtered = messages.filter(message => {
    ...
    if (hasOnlyWhitespaceTextContent(content)) {
      hasChanges = true
      logEvent('ncode_filtered_whitespace_only_assistant', {
        messageUUID:
...
function ensureNonEmptyAssistantContent(
  ...
  const result = messages.map((message, index) => {
    ...
    if (Array.isArray(content) && content.length === 0) {
      hasChanges = true
      logEvent('ncode_fixed_empty_assistant_content', {
        messageUUID:
          message.uuid as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
        messageIndex: index,
      })
...
export function filterOrphanedThinkingOnlyMessages(
  ...
  const filtered = messages.filter(msg => {
    ...
    logEvent('ncode_filtered_orphaned_thinking_message', {
      messageUUID:
...
export function ensureToolResultPairing(
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (nextMsg?.type === 'user') {
      ...
      if (patchedContent.length > 0) {
        ...
        result.push(
          checkStatsigFeatureGate_CACHED_MAY_BE_STALE('ncode_chair_sermon')
  ...
  if (repaired) {
    ...
    logEvent('ncode_tool_result_pairing_repaired', {
