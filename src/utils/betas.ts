...
function hasManagedPrincipalSession(): boolean {
  return getAuthRuntime().getCurrentSession().principalSource === 'managed_oauth'
}
...
function vertexModelSupportsWebSearch(model: string): boolean {
  const canonical = getCanonicalName(model)
  // Web search only supported on Anthropic Claude 4.0+ models on Vertex
  return (
...
// Context management is supported on Anthropic Claude 4+ models
...
export const getAllModelBetas = memoize((model: string): string[] => {
  ...
  // TTFT/TTLT/capacity; betas already flow to ncode_api_success for splitting.
  ...
  if (
    ...
    (isEnvTruthy(process.env.USE_CONNECTOR_TEXT_SUMMARIZATION) ||
      getFeatureValue_CACHED_MAY_BE_STALE('ncode_slate_prism', false))
  ) {
  ...
  const strictToolsEnabled =
    checkStatsigFeatureGate_CACHED_MAY_BE_STALE('ncode_tool_pear')
  ...
  const tokenEfficientToolsEnabled =
    !strictToolsEnabled &&
    getFeatureValue_CACHED_MAY_BE_STALE('ncode_amber_json_tools', false)
  if (
    includeFirstPartyOnlyBetas &&
  ...
  // Add web search beta for Vertex Anthropic Claude 4.0+ models only
